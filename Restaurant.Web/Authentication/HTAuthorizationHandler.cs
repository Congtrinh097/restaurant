...
namespace Restaurant.Web.Authentication
{
    ...
    public class HTAuthorizationHandler : IAuthorizationHandler
    {
        ...
        public HTAuthorizationHandler(InstanceRepository _InstanceRepository)
        {
            this.InstanceRepository = _InstanceRepository;
        }
        public Task HandleAsync(AuthorizationHandlerContext context)
        {
            ...
            var loginSession = InstanceRepository.UserLoginTokenRepository.GetAll()
                .Include(x => x.User)
            ...
            InstanceRepository.UserLoginTokenRepository.Save(loginSession);
            ...
            {
#if DEBUG
                var userRole = InstanceRepository.UserRoleRepository.GetAll().Where(x => x.UserId.HasValue && x.UserId.Value == loginSession.UserId).Select(x => x.Role).OrderByDescending(x => x.RoleType).FirstOrDefault();
        ...
        {
            return InstanceRepository.ClaimRepository.GetAll().Any(x => x.ClaimName == claim);
        }
        private void CreateNewClaim(string claim)
        {
            ...
            InstanceRepository.ClaimRepository.Save(entity);
            InstanceRepository.Commit();
        }

        private bool CheckClaims(string requireClaim, int userId)
        {
            return InstanceRepository.UserRoleRepository.GetAll()
